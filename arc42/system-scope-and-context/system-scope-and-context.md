...
| Entity | Description |
|-|-|
| EDDIE System | This is the software system within scope. The EDDIE system aims to offer energy-related data to services that are provided by the eligible party. Furthermore, the EDDIE system is envisioned to be comliant with interoperability initiatives, for e.g. Gaia-X, to allow for interoperability with external energy data spaces, i.e., similar software systems for sharing energy data. |
...
| Customer | The customer is a natural or legal person that consumes/produces energy, and is willing to share their energy consumption/production data. This data is used as input to the services. The customer can be motivated to allow their data to be used as the input for the services by gaining access to the output of the services. For example, a service may be processing residential energy consumption values to estimate consumption recommendations that reduce the energy bill as output.|
| EP Website | The EP Website (Eligible Party Website) is a website offered by the eligible party as an interface for the customers. This website presents the EDDIE Popup to the customer, which gathers all the necessary information to access their energy data. The EDDIE Popup is a user interface component provided by the EDDIE Framework that securely collects the customer permission and relevant details, enabling the retrieval of energy data from in-house metering devices and/or Regional Data-sharing Infrastructures. |
...
| External Energy Data Space| This is a software system similar to the EDDIE system that facilitates energy data sharing. The EDDIE system is expected to be compatible with external energy data spaces so that energy data can also be exchanged between different energy data spaces. |
...
The figure below shows a context diagram with the environment of the EDDIE System according to the [C4 model](https://c4model.com/). This diagram depicts the entities within scope with blue color, while the gray color indicates entities whose implementation is out of scope. As shown in the figure, the EDDIE system, being a system of systems, consists of three systems: the EDDIE Framework, AIIDA, and the Marketplace. 
...
| System | Description |
|-|-|
| EP Website | The EP Website present to the customer the EDDIE Popup to collect the necessary customer information for accessing the customer's energy data. The EDDIE Popup is provided by the EDDIE Framework, but it is displayed by the EP Website. This way, upon submission by the customer, the information of the EDDIE Popup is sent directly to the EDDIE Framework.|
...
| AIIDA | The Administration Interface for In-house Data Access (AIIDA) implements the main workflows regarding accessing near real-time data from in-house metering devices (e.g., smart meters), and sending it to the EDDIE Framework. |
