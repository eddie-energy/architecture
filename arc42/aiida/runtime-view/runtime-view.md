...
1. The AIIDA Regional Connector generates a Token and a QR code, and shows them on the EP Website.
1. The token and the QR code are shown to the customer.
1. The customer scans the QR code using the AIIDA Smartphone App, and gives permission to AIIDA to stream energy data.
...
After a connection has been established in AIIDA, the AIIDA Embedded App can stream energy data from Metering Devices to the AIIDA Regional Connection.
