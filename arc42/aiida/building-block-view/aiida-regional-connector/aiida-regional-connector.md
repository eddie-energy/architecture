...
The AIIDA Regional Connector implements many functionalities. First, it is used for acquiring the customer permission from the EP Website. Following the Regional Connector approach, the AIIDA Regional Connector provides the customer permission form to the customer via the EP Website, requesting the necessary customer information for the EDDIE Framework. Also, if the permission is revoked, e.g., from the EP Website, the AIIDA Regional Connector is notified and initiates the permission revocation that is propagated to AIIDA. When a customer permission is established, the AIIDA Regional Connector receives the energy data from the AIIDA Embedded App, and makes it available to the EDDIE Framework and its internal processes. The connection between the AIIDA Regional Connector and the AIIDA Embedded App is established via MQTT. 
