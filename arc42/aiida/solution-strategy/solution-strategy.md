...
## Deployability
As the in-house device hosting the AIIDA Embedded app, we use a Raspberry Pi single-board computer. The Raspberry Pi is chosen for being a widely used and cost effective device with large community, supporting several connectivity options and technologies. For example, supporting Docker which enables flexibility and portability is a very big advantage. Also, the Raspberry Pi community typically provides their solutions open-source which also helps with configurations and customizations.
