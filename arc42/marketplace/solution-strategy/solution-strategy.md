...
Apart from the frontend applications, the Marketplace system includes three more components with the following main functionalities: The Marketplace Application implements the necessary workflows, e.g., to register users, to search for data requests or services, etc. The Database stores information about the registered users, services, requests, etc. The IAM (Identity and Access Management) handles the authentication of registered users. Together, the components of the Marketplace implement the functionality to register users, submit data requests and services, search for services and requests, etc.