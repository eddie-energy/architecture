...
## Consumer revokes consent of eligible party from the Regional Data-sharing Infrastructure
...
1. The consent administrator revokes the consent. Since the involved components of this step do not belong to EDDIE, this step is out of scope. However, EDDIE depends on the timely execution of this step. Notably, EDDIE should implement a mechanism to verify that the consent administrator has indeed revoked the consent (if the consent administrator allows it, e.g., through an API).
