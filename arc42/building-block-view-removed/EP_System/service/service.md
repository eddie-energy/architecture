...
> [!WARNING]
> Page contains incorrect information about Kafka/streaming.

## Overview

Each eligible party can use Services for processing the historical validated and/or real-time data retrieved by the EDDIE Framework. The eligible party is responsible for the deployment and operation of the Services. However, it is not the goal of the EDDIE project to focus on developing Services. To connect with the EDDIE Framework and access the data, every Service needs to subscribe to the [Streaming Infrastructure](../../../deployment-view/deployment-view.md). The Streaming Infrastructure is a publish/subscribe messaging system that is implemented using Apache Kafka to offer to the Services the energy data in a CIM-compliant data model that is discussed [here](../../EDDIE_Framework/Interfaces/data_model/data_model.md). A diagram of these components is shown below.
